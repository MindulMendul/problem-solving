...
lli dis(pii a, pii b){
    lli xx=a.first-b.first, yy=a.second-b.second;
    return xx*xx+yy*yy;
}

// p2 를 기준으로, p1, p3 중에 p1이 더 가까운지 체크 
lli ccw_dis(pii a, pii b, pii c){
    lli xx1=a.first-b.first, yy1=a.second-b.second;
    lli xx2=b.first-c.first, yy2=b.second-c.second;
    return dis(a, b)<dis(b, c);
}
...
int main() {
    ...
    int e=1;
    lli ans=0;
    for(int s=0; s<__stack.size(); s++){
        while(dis(p[__stack[s]], p[__stack[(e+1)%__stack.size()]])>dis(p[__stack[s]], p[__stack[e]])){
            e=(e+1)%__stack.size();
        }
        int tmp_ans=dis(p[__stack[s]], p[__stack[e]]);
